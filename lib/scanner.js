...
Scanner.prototype.create = function(params, callback) {
  var args, encode, encoding, key;
  args = Array.prototype.slice.call(arguments);
  key = "/" + this.table + "/scanner";
  params = typeof args[0] === 'object' ? args.shift() : {};
  encoding = params.encoding === 'undefined' ? params.encoding : this.client.options.encoding;
  callback = args.shift();
  if (params.startRow) {
    params.startRow = utils.base64.encode(params.startRow, encoding);
  }
  if (params.endRow) {
    params.endRow = utils.base64.encode(params.endRow, encoding);
  }
  if (params.column) {
  ...
  return this.client.connection.put(key, params, (function(_this) {
    return function(error, data, response) {
      var id;
      if (error) {
        return callback.apply(_this, [error, null]);
      }
      id = /scanner\/(\w+)$/.exec(response.headers.location)[1];
      _this.id = id;
      return callback.apply(_this, [null, id]);
    };
  })(this));
};
  ...
  return this.client.connection.get(key, (function(_this) {
    return function(error, data, response) {
      ...
      if (response && response.statusCode === 204) {
        return callback.apply(_this, [null, null]);
      }
      if (error) {
        return callback.apply(_this, [error, null]);
      }
      ...
      return callback.apply(_this, [null, cells]);
    };
  })(this));
};
...
Scanner.prototype["delete"] = function(callback) {
  var key;
  key = "/" + this.table + "/scanner/" + this.id;
  return this.client.connection["delete"](key, (function(_this) {
